	...
	for(int playerNumber = 0; playerNumber < MaxPlayerNmb; playerNumber++)
	{
		...
		if(!playerRenderData.isRespawning)
		{
			const auto targetDir = neko::Vec2f(
					static_cast<neko::Scalar>((float)playerInput.targetDirX),
					static_cast<neko::Scalar>((float)playerInput.targetDirY));
			const bool isShooting = targetDir.Length() > static_cast<neko::Scalar>((float)PlayerCharacter::deadZone) && !playerCharacter.IsReloading();
			if(isShooting)
			{
			...
			case PlayerRenderState::FALL:
			{
				...
				{
					if (neko::Scalar{ (float)playerInput.moveDirY } > PlayerCharacter::JetBurstThreshold)
					{
			...
			case PlayerRenderState::SHOOT:
			{
				...
				else
				{
					...
					{
						if ((targetDir.x > static_cast<neko::Scalar>((float)PlayerCharacter::deadZone) && !playerRenderData.faceRight) ||
							(targetDir.x < static_cast<neko::Scalar>((float)-PlayerCharacter::deadZone) && playerRenderData.faceRight))
						{
							playerRenderData.faceRight = !playerRenderData.faceRight;
						}
					}
			...
			case PlayerRenderState::JET:
			{
				...
				if (neko::Scalar{ (float)playerInput.moveDirY } < PlayerCharacter::ReactorThreshold)
				{
					SwitchToState(PlayerRenderState::FALL, playerNumber);
				}
				if(!playerCharacter.jumpTimer.Over())
