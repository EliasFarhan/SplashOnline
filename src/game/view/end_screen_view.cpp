...
void EndScreenView::Draw()
{
	...
	for(int i = 0; i < MaxPlayerNmb; i++)
	{
		...
		neko::Scalar x = neko::Scalar{(float)i+1}*size.x/neko::Scalar(playerCount_+1)-size.x/neko::Scalar{2};
		neko::Scalar y = neko::Scalar{(float)(playerCount_+1)-playerRanks_[i]}*size.y/neko::Scalar(playerCount_+1)-size.y/neko::Scalar{2};
