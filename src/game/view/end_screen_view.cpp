...
void EndScreenView::SetPlayerScore(const std::array<int, MaxPlayerNmb>& playerScores)
{
	...
	for(int i = 0; i < MaxPlayerNmb; i++)
	{
		if(IsValid(i))
		{
			playerRanks_[i] = sixit::guidelines::narrow_cast<int>(std::distance(sortedPlayerScores.begin(), std::find(sortedPlayerScores.begin(), sortedPlayerScores.end(), playerScores[i])))+1;
...
void EndScreenView::Draw()
{
	...
	for(int i = 0; i < MaxPlayerNmb; i++)
	{
		...
		neko::Scalar x = neko::Scalar{sixit::guidelines::precision_cast<float>(i+1)}*size.x/neko::Scalar(sixit::guidelines::precision_cast<float>(playerCount_+1))-size.x/neko::Scalar{2};
		neko::Scalar y = neko::Scalar{ sixit::guidelines::precision_cast<float>(playerCount_+1-playerRanks_[i])}*size.y/neko::Scalar(sixit::guidelines::precision_cast<float>(playerCount_+1))-size.y/neko::Scalar{2};
