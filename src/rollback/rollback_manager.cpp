...
Checksum<static_cast<int>(BulletChecksumIndex::LENGTH) + static_cast<int>(PlayerChecksumIndex::LENGTH)>
RollbackManager::ConfirmLastFrame()
{
    const auto inputs = GetInputs(neko::Max(lastConfirmFrame_, static_cast<int16_t>(0)));
...
{
    int16_t lastReceived = std::numeric_limits<int16_t>::max();
    for (int playerNumber = 0; playerNumber < MaxPlayerNmb; playerNumber++)
    {
        if (!inputDatas_[playerNumber].isValid) continue;
        if (lastReceived > inputDatas_[playerNumber].lastReceivedFrame)
        {
            lastReceived = inputDatas_[playerNumber].lastReceivedFrame;
        }
    }
    return lastReceived;
}
...
std::pair<std::array<PlayerInput, MaxPlayerInputNmb>, uint8_t> RollbackManager::GetInputs(uint8_t playerNumber,
                                                                                          uint16_t currentFrame) const
{
    std::array<PlayerInput, MaxPlayerInputNmb> inputsTmp{};
    uint16_t size = sixit::guidelines::narrow_cast<uint16_t>(
        neko::Max(currentFrame - neko::Max(lastConfirmFrame_, static_cast<int16_t>(0)) + 1, 1));
    uint16_t firstFrame = neko::Max(lastConfirmFrame_, static_cast<int16_t>(0));
...
bool RollbackManager::IsValid(int playerNumber) const { return inputDatas_[playerNumber].isValid; }
const GameSystems& RollbackManager::GetGameSystems() const { return confirmFrameGameSystems_; }
void RollbackManager::Begin() { confirmFrameGameSystems_.Begin(); }
void RollbackManager::SetDirty(bool dirty) { isDirty_ = dirty; }
int RollbackManager::GetLastReceivedFrame(int playerNumber) const
{
    return inputDatas_[playerNumber].lastReceivedFrame;
}
bool IsValid(int playerNumber) { return instance->IsValid(playerNumber); }
} // namespace splash
