...
public:
    ...
    [[nodiscard]] int GetSystemIndex() const override { return systemIndex; }
    void SetSystemIndex(int index) override { systemIndex = index; }
...
void NetworkClientImpl::clientErrorReturn(int errorCode) { LogError(fmt::format("Client Error, code: {}", errorCode)); }
void NetworkClientImpl::warningReturn(int warningCode) { LogWarning(fmt::format("Warning, code: {}", warningCode)); }
void NetworkClientImpl::serverErrorReturn(int errorCode) { LogError(fmt::format("Server Error, code: {}", errorCode)); }
void NetworkClientImpl::joinRoomEventAction(int playerNr, const ExitGames::Common::JVector<int>& playernrs,
                                            const ExitGames::LoadBalancing::Player& player)
{
    ...
    LogDebug(fmt::format("Join Room Event: playerNr: {} player name: {}", playerNr,
                         player.getName().ASCIIRepresentation().cstr()));
...
void NetworkClientImpl::customEventAction(int playerNr, nByte eventCode, const ExitGames::Common::Object& eventContent)
{
    ...
    {
    case PacketType::DESYNC:
    {
        if (state_.load(std::memory_order_acquire) == NetworkClient::State::IN_GAME)
        {
            state_.store(NetworkClient::State::ERROR_ROOM, std::memory_order_release);
            LogDebug("Received Desync");
        }
        break;
    }
        ...
        if (isMaster_)
        {
            ...
            networkTasks_.emplace_back(
                [serializer, playerNr]
                {
                    ...
                    options.setTargetPlayers(&playerNr, 1);
                    client.opRaiseEvent(false, serializer, static_cast<nByte>(PacketType::PING), options);
                });
        }
        else
        {
            ...
            if (lastPingDt < 0.0f)
            {
                const auto currentPingTime = SDL_GetPerformanceCounter();
                const auto freq = SDL_GetPerformanceFrequency();
                const auto newPing = static_cast<float>(static_cast<double>(currentPingTime-previousPingTime)/freq);
                ping = 0.5f * ping + 0.5f * newPing;
            }
            if (pingPacket.masterTime != PingPacket::time_type{0.0f})
            {
...
void NetworkClientImpl::connectReturn(int errorCode, const ExitGames::Common::JString& errorString,
                                      const ExitGames::Common::JString& region,
                                      const ExitGames::Common::JString& cluster)
{
    (void)errorCode;
    (void)errorString;
    LogDebug(fmt::format("Connect Return: region: {} cluster: {}", region.ASCIIRepresentation().cstr(),
                         cluster.ASCIIRepresentation().cstr()));
    state_.store(NetworkClient::State::CONNECTED_TO_MASTER, std::memory_order_release);
}
void NetworkClientImpl::disconnectReturn(void) { LogDebug("Disconnect Return"); }
                                           ...
                                           const ExitGames::Common::JVector<ExitGames::Common::JString>& regionsServers)
{
    ...
    {
        regionsTmp.emplace_back(regions[i].ASCIIRepresentation().cstr(),
                                regionsServers[i].ASCIIRepresentation().cstr());
    }
            ...
            if (ImGui::Button("Start Game"))
            {
                ...
                networkTasks_.emplace_back(
                    []
                    {
                        ...
                        ExitGames::LoadBalancing::RaiseEventOptions options{};
                        client.opRaiseEvent(true, static_cast<nByte>(0), static_cast<nByte>(PacketType::START_GAME),
                                            options);
                    });
            }
        }
...
void NetworkClientUi::SetGuiIndex(int index) { guiIndex_ = index; }
int NetworkClientUi::GetGuiIndex() const { return guiIndex_; }
static void RunNetwork()
{
...
uint8_t NetworkClient::GetPlayerIndex() { return localPlayerIndex_; }
...
void SendPingPacket()
{
    ...
    previousPingTime = SDL_GetPerformanceCounter();
    lastPingDt = -1.0f;
}

bool NetworkClient::IsMaster() { return isMaster_; }
NetworkClient::State NetworkClient::GetState() { return state_.load(std::memory_order_acquire); }
bool NetworkClient::IsValid() { return isValid_; }
} // namespace splash
